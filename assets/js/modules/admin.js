...
async function createNotification({ recipientUserId = null, recipientEmail = null, recipientRole = null, title = 'Notification', message = '', type = 'info', relatedId = null } = {}) {
  ...
  async function insertNotification(payload) {
    const attempts = [
      ...
      },
      {
        ...
        receiver_email: payload.recipient_email,
  ...
  try {
    ...
    if (!recipientUserId && !recipientEmail && recipientRole) {
...
function initAdminActions() {
  ...
  claimsTable?.addEventListener('click', async (e) => {
    ...
    try {
      ...
      if (targetReportId) {
        const { data: reportData } = await sb.from('item_reports').select('id,item_name,user_id,location,status').eq('id', targetReportId).maybeSingle();
...
async function deleteRequestTicket(ticketId) {
  ...
  try {
    await createNotification({
      recipientRole: 'system_admin',
      title: 'Request Ticket Deleted',
      message: ticket?.message ? `Deleted ticket: ${ticket.message}` : 'A request ticket was deleted.',
      type: 'ticket',
      relatedId: ticketId
    });
  } catch (_) {}
  ...
  const rows = archives.length ? archives.map(a => {
    ...
    return `<div class="border-b border-slate-200 p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <button class="rounded-xl bg-emerald-700 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-800 transition" onclick="restoreArchivedRecord('${escapeHtml(a.id)}')">Restore</button>
        <button class="rounded-xl bg-red-600 px-3 py-2 text-xs font-bold text-white hover:bg-red-700 transition" onclick="permanentlyDeleteArchivedRecord('${escapeHtml(a.id)}')">Delete</button>
      </div>
    </div>`;
...
async function permanentlyDeleteArchivedRecord(id){
  ...
  try {
    ...
    if (error) throw error;
    hideLoading();
    showSuccess('Archived report permanently deleted.', { position: 'top-right' });
    await openRecoverDataModal();
    await renderSystemAdmin();
  } catch (err) {
    hideLoading();
