...
async function fetchCurrentUserNotifications(limit = 25) {
  ...
  if (scope.uid) {
    ...
    attempts.push(() => sb.from('notifications').select('*').eq('user_id', scope.uid).order('created_at', { ascending: false }).limit(limit));
    attempts.push(() => sb.from('notifications').select('*').eq('receiver_id', scope.uid).order('created_at', { ascending: false }).limit(limit));
  }

  if (scope.email) {
    attempts.push(() => sb.from('notifications').select('*').eq('recipient_email', scope.email).order('created_at', { ascending: false }).limit(limit));
    attempts.push(() => sb.from('notifications').select('*').eq('email', scope.email).order('created_at', { ascending: false }).limit(limit));
    attempts.push(() => sb.from('notifications').select('*').eq('receiver_email', scope.email).order('created_at', { ascending: false }).limit(limit));
  }

  if (scope.role) {
    attempts.push(() => sb.from('notifications').select('*').eq('recipient_role', scope.role).order('created_at', { ascending: false }).limit(limit));
    attempts.push(() => sb.from('notifications').select('*').eq('role', scope.role).order('created_at', { ascending: false }).limit(limit));
    attempts.push(() => sb.from('notifications').select('*').eq('target_role', scope.role).order('created_at', { ascending: false }).limit(limit));
  }
...
async function markCurrentNotificationRead(notificationId) {
  ...
  try {
    ...
    try { await sb.from('notifications').update({ read: true }).eq('id', notificationId); }
    catch (_) { console.warn('Unable to mark notification as read:', err.message); showError('Unable to update notification. Please try again.'); }
  }
}
...
async function deleteCurrentNotification(notificationId) {
  ...
  try {
    ...
    // First attempt: delete by ID. RLS will still protect ownership.
    let result = await sb.from('notifications').delete().eq('id', notificationId);
    if (result.error) throw result.error;

    // Verify if row is gone or inaccessible to current user.
    const check = await sb.from('notifications').select('id').eq('id', notificationId).maybeSingle();
    if (!check.data || check.error) {
      showSuccess('Notification deleted.');
      return true;
    }

    // Fallback: try scoped delete using common recipient columns.
    const attempts = [];
    if (scope.uid) {
      attempts.push(() => sb.from('notifications').delete().eq('id', notificationId).eq('recipient_user_id', scope.uid));
      attempts.push(() => sb.from('notifications').delete().eq('id', notificationId).eq('user_id', scope.uid));
      attempts.push(() => sb.from('notifications').delete().eq('id', notificationId).eq('receiver_id', scope.uid));
    }
    if (scope.email) {
      attempts.push(() => sb.from('notifications').delete().eq('id', notificationId).eq('recipient_email', scope.email));
      attempts.push(() => sb.from('notifications').delete().eq('id', notificationId).eq('email', scope.email));
      attempts.push(() => sb.from('notifications').delete().eq('id', notificationId).eq('receiver_email', scope.email));
    }
    ...
    for (const attempt of attempts) {
      try {
        ...
        const verify = await sb.from('notifications').select('id').eq('id', notificationId).maybeSingle();
    ...
    showError('Notification was not deleted. Please run the latest notification delete policy in Supabase.');
    return false;
  } catch (err) {
    console.warn('Unable to delete notification:', err.message);
    showError('Unable to delete notification. Please run the latest notification delete policy in Supabase, then refresh.');
    return false;
  }
}
...
function initPasswordVisibilityToggles() {
  document.querySelectorAll('input[type="password"]').forEach(input => {
    ...
    // Some pages, like reset-password.html and System Admin modals, already
    // include their own eye button. Do not auto-inject another one.
    const existingToggle = parent.querySelector(`[data-toggle-password="${input.id}"]`);
    if (existingToggle) {
