...
/* =========================================================
   GCFind v2.58 FORCE-FIRST BUTTON ACTION FIX
   This is intentionally placed at the TOP of admin.js.
   It runs before old broken handlers and stops them only after handling.
   ========================================================= */
(function gcfindV258ForceFirstActions(){
  if (window.__gcfindV258ForceFirstActions) return;
  window.__gcfindV258ForceFirstActions = true;

  const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

  function isUuid(id){ return UUID_RE.test(String(id || '').trim()); }
  ...
  function removeNode(btn){
    const node = btn?.closest('article, tr, .card, .ticket-card, .request-ticket-card, .notification-card, .gc-notification-card, .border-t, .p-4');
    if (node) node.remove();
  }

  async function confirmSafe(msg){
    try { if (typeof appConfirm === 'function') return await appConfirm(msg); } catch(_) {}
    return window.confirm(msg);
  }

  function ok(msg){
    try { showSuccess(msg, { position: 'top-right' }); } catch(_) {}
  }

  function err(msg){
    try { showError(msg, { position: 'top-right', duration: 7000 }); } catch(_) { alert(msg); }
  }

  async function refreshSoft(){
    try { if (typeof refreshGlobalNotifications === 'function') await refreshGlobalNotifications(); } catch(_) {}
    try { if (typeof renderStaffPanel === 'function' && (document.getElementById('staffPanelMount') || location.pathname.includes('staff'))) await renderStaffPanel(); } catch(_) {}
    try { if (typeof renderSystemAdmin === 'function' && location.pathname.includes('system-admin')) await renderSystemAdmin(); } catch(_) {}
  }

  async function dbUpdate(table, payload, id){
    if (!window.sb || !id || !isUuid(id)) return { skipped: true };
    try { return await sb.from(table).update(payload).eq('id', id); }
    catch(e) { return { error: e }; }
  }

  async function dbDelete(table, id){
    if (!window.sb || !id || !isUuid(id)) return { skipped: true };
    try { return await sb.from(table).delete().eq('id', id); }
    catch(e) { return { error: e }; }
  }

  async function dbSelectOne(table, id){
    if (!window.sb || !id || !isUuid(id)) return { data: null, error: null, skipped: true };
    try { return await sb.from(table).select('*').eq('id', id).maybeSingle(); }
  ...
  async function handleNotificationDelete(id, btn){
    ...
    try { if (typeof deleteStaffNotificationLocally === 'function') deleteStaffNotificationLocally(id); } catch(_) {}
    const res = await dbDelete('notifications', id);
    if (res.error) console.warn('notification delete DB blocked:', res.error.message || res.error);
    removeNode(btn);
  ...
  async function handleNotificationRead(id, btn){
    ...
    const res = await dbUpdate('notifications', { is_read: true }, id);
    if (res.error) console.warn('notification read DB blocked:', res.error.message || res.error);
    btn?.remove();
  ...
  async function handleClaim(claimId, status, btn){
    const yes = await confirmSafe(`${status === 'Approved' ? 'Approve' : 'Reject'} this claim?`);
    if (!yes) return;
    const payload = { status };
    try { payload.updated_at = new Date().toISOString(); } catch(_) {}
    const res = await dbUpdate('claim_requests', payload, claimId);
    if (res.error) {
      err(res.error.message || 'Claim update failed.');
      return;
    }
    removeNode(btn);
    ok(`Claim marked as ${status}.`);
    await refreshSoft();
  }

  async function handleTicketDelete(id, btn){
    const yes = await confirmSafe('Delete this request ticket?');
    if (!yes) return;
    addLocal('gcfind_deleted_request_tickets', id);
    addLocal('gcfind_hidden_request_tickets', id);
    const res = await dbDelete('request_tickets', id);
    if (res.error) console.warn('ticket delete DB blocked:', res.error.message || res.error);
    removeNode(btn);
    ok('Request ticket deleted.');
    await refreshSoft();
  }

  async function handleReportDelete(id, btn){
    const yes = await confirmSafe('Delete this submitted report?');
    if (!yes) return;
    const res = await dbDelete('item_reports', id);
    if (res.error) {
      err(res.error.message || 'Report delete failed.');
      return;
    }
    removeNode(btn);
    ok('Report deleted.');
  ...
  async function handleReportStatus(id, status, btn){
    const yes = await confirmSafe(`Mark this report as ${status}?`);
    if (!yes) return;
    const payload = { status };
    try { payload.updated_at = new Date().toISOString(); } catch(_) {}
    const res = await dbUpdate('item_reports', payload, id);
    if (res.error) {
      err(res.error.message || 'Report update failed.');
      return;
    }
    ok(`Report marked as ${status}.`);
    await refreshSoft();
  }

  async function handleArchiveDelete(id, btn){
    const yes = await confirmSafe('Permanently delete this archived record?');
    if (!yes) return;
    const res = await dbDelete('deleted_records_archive', id);
    if (res.error) {
      err(res.error.message || 'Archive delete failed.');
      return;
    }
    removeNode(btn);
    ok('Archived record deleted.');
  }

  async function handleArchiveRestore(id, btn){
    const yes = await confirmSafe('Restore this archived record?');
    if (!yes) return;

    const row = await dbSelectOne('deleted_records_archive', id);
    if (row.error) {
      err(row.error.message || 'Unable to read archived record.');
      return;
    }
    const archive = row.data;
    if (!archive) {
      err('Archived record not found.');
      return;
    }

    const table = archive.table_name || archive.source_table || 'item_reports';
    ...
    if (!record || typeof record !== 'object') {
      err('Archive has no restorable record_data.');
      return;
    }

    // Remove fields that commonly break restore.
    record = { ...record };
    delete record.id;
    ...
    try {
      let insert = await sb.from(table).insert(record);
      if (insert.error && /foreign key|user_id/i.test(insert.error.message || '')) {
        delete record.user_id;
        delete record.reporter_id;
        insert = await sb.from(table).insert(record);
      }
      if (insert.error) throw insert.error;

      await dbDelete('deleted_records_archive', id);
      removeNode(btn);
      ok('Record restored.');
      await refreshSoft();
    } catch(e) {
  ...
  document.addEventListener('click', async function(e){
    ...
    // Notifications: supports normal and staff synthetic IDs.
    const notifRead = btn.dataset.notificationRead || btn.dataset.staffAlertRead;
    const notifDel = btn.dataset.notificationDelete || btn.dataset.staffAlertDelete;
    if (notifRead) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      await handleNotificationRead(notifRead, btn);
      return;
    }
    if (notifDel) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      const yes = await confirmSafe('Delete this notification?');
      if (!yes) return;
      await handleNotificationDelete(notifDel, btn);
      return;
    }
    ...
    if (btn.dataset.staffClaim && btn.dataset.claimId) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      await handleClaim(btn.dataset.claimId, btn.dataset.staffClaim === 'approve' ? 'Approved' : 'Rejected', btn);
      return;
    }

    // Other claim action variants.
    if (btn.dataset.claimAction && btn.dataset.claimId) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      const action = String(btn.dataset.claimAction).toLowerCase();
      await handleClaim(btn.dataset.claimId, action === 'approve' ? 'Approved' : 'Rejected', btn);
      return;
    }
    ...
    // Submitted Reports / item reports.
    ...
    if (reportId && reportAction) {
      ...
      const action = String(reportAction).toLowerCase();
      if (action === 'delete') await handleReportDelete(reportId, btn);
      else await handleReportStatus(reportId, map[action] || reportAction, btn);
      return;
    }
    ...
    if (btn.dataset.action === 'delete' && btn.dataset.table && btn.dataset.id) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      if (btn.dataset.table === 'notifications') await handleNotificationDelete(btn.dataset.id, btn);
      else if (btn.dataset.table === 'request_tickets') await handleTicketDelete(btn.dataset.id, btn);
      else if (btn.dataset.table === 'item_reports') await handleReportDelete(btn.dataset.id, btn);
      else await dbDelete(btn.dataset.table, btn.dataset.id).then(() => removeNode(btn));
      return;
    }
  }, true);
})();
...
async function renderStaffPanel() {
  ...
  const pendingClaims = claimResult.rows || [];
  ...
  claimsBox.querySelectorAll('[data-staff-claim]').forEach(btn => btn.addEventListener('click', async () => {
    const claimId = btn.dataset.claimId;
    const action = btn.dataset.staffClaim;
    const nextStatus = action === 'approve' ? 'Approved' : 'Rejected';
    const claim = pendingClaims.find(c => String(c.id) === String(claimId));
    const ok = await appConfirm(`${nextStatus === 'Approved' ? 'Approve' : 'Reject'} this claim request?`);
    if (!ok) return;

    try {
      showLoading(`${nextStatus === 'Approved' ? 'Approving' : 'Rejecting'} claim...`);

      // IMPORTANT:
      // Do not chain .select() here. Some Supabase RLS setups allow UPDATE but block SELECT,
      // which makes the UI think approval failed even when the update was applied.
      ...
      await createAuditLog(`Staff Claim ${nextStatus}`, 'claim_request', claimId, claim?.message || nextStatus);

      try {
        await createNotification({
          recipientRole: 'admin',
          title: `Claim ${nextStatus} by Staff`,
          message: `${user.name || user.email || 'Faculty / Staff'} marked a claim request as ${nextStatus}.`,
          type: 'claim',
          relatedId: claimId
        });
      } catch (_) {}
      ...
      try {
        if (claim?.claimantId) {
          await createNotification({
            ...
            title: `Your claim request was ${nextStatus}`,
            message: nextStatus === 'Approved'
              ? 'Your claim request was reviewed and approved by Faculty / Staff. Please coordinate with the Security / Lost & Found Office.'
              : 'Your claim request was reviewed and rejected by Faculty / Staff. Please coordinate with the Security / Lost & Found Office for clarification.',
            type: 'claim',
            relatedId: claimId
          });
        } else if (claim?.claimantEmail) {
          await createNotification({
      ...
      hideLoading();
      showSuccess(`Claim marked as ${nextStatus}.`, { position: 'top-right' });

      // Remove immediately from pending UI so the panel feels correct even before realtime refresh.
      btn.closest('article')?.remove();
      ...
      hideLoading();
      console.warn('Staff claim update failed:', err);
      showError(err.message || 'Unable to update claim request. Run SUPABASE_V2_52_ACTIONS_FIXED.sql in Supabase, then refresh.', { position: 'top-right', duration: 6000 });
    }
  }));
...
function openCreateAccountModal() {
  openSystemAdminModal(`
    ...
    <p class="mt-1 text-sm text-slate-600">Create a new GCFind user account. A secure password setup link will be sent to the user.</p>
    <form id="sysCreateAccountForm">
      ...
      <div class="gc-muted-note">After creating the account, the system will send a password setup link to the user's email.</div>
  ...
  document.getElementById('sysCreateAccountForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (created) {
        ...
        try {
          await callAdminApi('/api/admin-reset-password', { email: payload.email, redirectTo });
        } catch (_) {
          const { error: resetError } = await sb.auth.resetPasswordForEmail(payload.email, { redirectTo });
...
(function gcfindFinalActionStabilizer(){
  ...
  document.addEventListener('click', async (event) => {
    ...
    const readBtn = getBtn(target, '[data-notification-read]');
    if (readBtn) {
      event.preventDefault();
      event.stopPropagation();
      await markNotificationReadHard(readBtn.dataset.notificationRead, readBtn);
      return;
    }

    const delNotifBtn = getBtn(target, '[data-notification-delete]');
    if (delNotifBtn) {
      event.preventDefault();
      event.stopPropagation();
      const ok = await safeConfirm('Delete this notification?');
      if (!ok) return;
      await deleteNotificationHard(delNotifBtn.dataset.notificationDelete, delNotifBtn);
      return;
    }

    const staffClaimBtn = getBtn(target, '[data-staff-claim]');
    if (staffClaimBtn) {
      event.preventDefault();
      event.stopPropagation();
      const action = staffClaimBtn.dataset.staffClaim;
      await updateClaimHard(staffClaimBtn.dataset.claimId, action === 'approve' ? 'Approved' : 'Rejected', staffClaimBtn);
      return;
    }
...
/* =========================================================
   GCFind v2.57 STAFF CLAIM + NOTIFICATION FINAL PATCH
   - Prevents DB PATCH/DELETE for synthetic staff ids.
   - Delegates click handlers globally, so buttons work even after rerender.
   - Uses actual Supabase tables: notifications + claim_requests.
   ========================================================= */
(function gcfindV257StaffClaimNotificationPatch(){
  if (window.__gcfindV257StaffClaimNotificationPatch) return;
  window.__gcfindV257StaffClaimNotificationPatch = true;
  ...
  function removeCard(btn) {
    const card = btn?.closest('article, tr, .card, .ticket-card, .request-ticket-card, .notification-card, .gc-notification-card');
    if (card) card.remove();
  }

  async function maybeRefreshStaff() {
    try { if (typeof refreshGlobalNotifications === 'function') await refreshGlobalNotifications(); } catch (_) {}
    try {
      const onStaff = document.getElementById('staffPanelMount') || document.getElementById('staffPendingClaims') || document.getElementById('staffNotifications');
      if (onStaff && typeof renderStaffPanel === 'function') await renderStaffPanel();
    } catch (_) {}
  }

  async function confirmSafe(message) {
    try {
      if (typeof appConfirm === 'function') return await appConfirm(message);
    } catch (_) {}
    return window.confirm(message);
  ...
  async function markReadSafe(id, btn) {
    if (!id) return;
    addLocalList('gcfind_staff_read_alerts', id);
    addLocalList('gcfind_read_notifications', id);

    if (isUuid(id) && window.sb) {
      try {
        const { error } = await sb.from('notifications').update({ is_read: true }).eq('id', id);
        if (error) console.warn('Notification mark-read blocked:', error.message);
      } catch (err) {
        console.warn('Notification mark-read error:', err.message);
      }
    }
    ...
    card?.classList.add('bg-white');
    btn?.remove();
    try { showSuccess('Notification marked as read.', { position: 'top-right' }); } catch (_) {}
  }

  async function deleteNotificationSafe(id, btn) {
    if (!id) return;
    addLocalList('gcfind_staff_deleted_alerts', id);
    addLocalList('gcfind_deleted_staff_notifications', id);
    try { if (typeof deleteStaffNotificationLocally === 'function') deleteStaffNotificationLocally(id); } catch (_) {}
    addLocalList('gcfind_staff_read_alerts', id);
    addLocalList('gcfind_deleted_notifications', id);
    addLocalList('gcfind_hidden_notifications', id);
    try { if (typeof hidePanelNotificationLocally === 'function') hidePanelNotificationLocally(id); } catch (_) {}
    try { if (typeof hideNotificationLocally === 'function') hideNotificationLocally(id); } catch (_) {}
    ...
    if (isUuid(id) && window.sb) {
      try {
        ...
        if (error) console.warn('Notification delete blocked:', error.message);
      } catch (err) {
        console.warn('Notification delete error:', err.message);
      }
    }

    removeCard(btn);
    try { showSuccess('Notification deleted.', { position: 'top-right' }); } catch (_) {}
  }

  async function updateClaimSafe(claimId, nextStatus, btn) {
    if (!claimId) return;
    const ok = await confirmSafe(`${nextStatus === 'Approved' ? 'Approve' : 'Reject'} this claim request?`);
    if (!ok) return;

    try { if (typeof showLoading === 'function') showLoading(`${nextStatus === 'Approved' ? 'Approving' : 'Rejecting'} claim...`); } catch (_) {}

    let updateError = null;
    try {
      const payload = { status: nextStatus };
      try { payload.updated_at = new Date().toISOString(); } catch (_) {}
      const { error } = await sb.from('claim_requests').update(payload).eq('id', claimId);
      updateError = error;
    } catch (err) {
      updateError = err;
    }
    ...
    if (updateError) {
      console.warn('Claim update failed:', updateError.message || updateError);
      try { showError(updateError.message || 'Unable to update claim request.', { position: 'top-right', duration: 7000 }); }
      catch (_) { alert(updateError.message || 'Unable to update claim request.'); }
      return;
    }

    removeCard(btn);
    try { showSuccess(`Claim marked as ${nextStatus}.`, { position: 'top-right' }); } catch (_) {}

    // best effort: notify dashboards, but never block the button success
    try {
      if (typeof createNotification === 'function') {
        await createNotification({
          recipientRole: 'admin',
          title: `Claim ${nextStatus} by Staff`,
          message: `Faculty / Staff marked a claim request as ${nextStatus}.`,
          type: 'claim',
          relatedId: claimId
        });
        await createNotification({
    ...
    await maybeRefreshStaff();
  }

  // capture phase, so this works even if older handlers are broken
  document.addEventListener('click', async function(e) {
    const staffClaimBtn = e.target.closest?.('[data-staff-claim]');
    if (staffClaimBtn) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      const action = staffClaimBtn.dataset.staffClaim;
      const claimId = staffClaimBtn.dataset.claimId;
      await updateClaimSafe(claimId, action === 'approve' ? 'Approved' : 'Rejected', staffClaimBtn);
      return;
    }

    const staffReadBtn = e.target.closest?.('[data-staff-alert-read]');
    if (staffReadBtn) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      await markReadSafe(staffReadBtn.dataset.staffAlertRead, staffReadBtn);
      return;
    }

    const staffDeleteBtn = e.target.closest?.('[data-staff-alert-delete]');
    if (staffDeleteBtn) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      const ok = await confirmSafe('Delete this notification?');
      if (!ok) return;
      await deleteNotificationSafe(staffDeleteBtn.dataset.staffAlertDelete, staffDeleteBtn);
      return;
    }

    const notifReadBtn = e.target.closest?.('[data-notification-read]');
    if (notifReadBtn) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      await markReadSafe(notifReadBtn.dataset.notificationRead, notifReadBtn);
      return;
    }

    const notifDeleteBtn = e.target.closest?.('[data-notification-delete]');
    if (notifDeleteBtn) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      const ok = await confirmSafe('Delete this notification?');
      if (!ok) return;
      await deleteNotificationSafe(notifDeleteBtn.dataset.notificationDelete, notifDeleteBtn);
      return;
    }
  }, true);
})();
...
/* =========================================================
   GCFind v2.57 EXTRA ACTION BRIDGE
   Covers buttons that were rendered as UI but missed by older listeners.
   ========================================================= */
(function gcfindV257ExtraActionBridge(){
  if (window.__gcfindV257ExtraActionBridge) return;
  window.__gcfindV257ExtraActionBridge = true;

  async function confirmSafe(message) {
    try { if (typeof appConfirm === 'function') return await appConfirm(message); } catch (_) {}
    return window.confirm(message);
  }

  function removeCard(btn) {
    btn?.closest('article, tr, .card, .ticket-card, .request-ticket-card, .notification-card, .border-t, .p-4')?.remove();
  }
  ...
  async function refreshAdminLike() {
    try { if (typeof refreshGlobalNotifications === 'function') await refreshGlobalNotifications(); } catch (_) {}
    try { if (typeof renderAdminTicketNotifications === 'function') await renderAdminTicketNotifications(); } catch (_) {}
    try { if (typeof renderSystemAdmin === 'function' && (document.getElementById('systemAdminMount') || document.getElementById('systemAdminDashboard'))) await renderSystemAdmin(); } catch (_) {}
    try { if (typeof renderAdminReports === 'function' && document.getElementById('adminReportsBody')) await renderAdminReports(); } catch (_) {}
  }

  async function deleteFromTable(table, id, btn, message) {
    if (!table || !id) return;
    const ok = await confirmSafe(message || 'Delete this item?');
    if (!ok) return;
    let error = null;
    try {
    ...
    if (error) {
      console.warn(`${table} delete failed:`, error.message || error);
      try { showError(error.message || `Unable to delete from ${table}.`, { position: 'top-right', duration: 7000 }); } catch (_) {}
      return;
    }
    removeCard(btn);
    await notifySuccess('Deleted successfully.');
    await refreshAdminLike();
  }

  async function updateReportStatus(id, status, btn) {
    if (!id) return;
    const ok = await confirmSafe(`Mark this report as ${status}?`);
    if (!ok) return;
    const { error } = await sb.from('item_reports').update({ status, updated_at: new Date().toISOString() }).eq('id', id);
    if (error) {
      try { showError(error.message, { position: 'top-right', duration: 7000 }); } catch (_) {}
      return;
    }
    try { btn.closest('tr')?.querySelector('td:nth-child(5)')?.replaceChildren(document.createTextNode(status)); } catch (_) {}
    await notifySuccess(`Report marked as ${status}.`);
    await refreshAdminLike();
  }
  ...
  document.addEventListener('click', async (e) => {
    ...
    // Request ticket delete variants
    const ticketId =
      b.dataset.ticketDelete ||
      b.dataset.adminTicketDelete ||
      b.dataset.requestTicketDelete ||
      b.dataset.adminTicketUpdateDelete;
    if (ticketId) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      await deleteFromTable('request_tickets', ticketId, b, 'Delete this request ticket?');
      return;
    }

    // Archive restore/delete variants
    const archiveRestore = b.dataset.archiveRestore;
    const archiveDelete = b.dataset.archiveDelete;
    if (archiveRestore) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      if (typeof restoreArchivedRecord === 'function') {
        await restoreArchivedRecord(archiveRestore);
      } else {
        try { showError('Restore function is not loaded.', { position: 'top-right' }); } catch (_) {}
      }
      return;
    }
    if (archiveDelete) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      if (typeof permanentlyDeleteArchivedRecord === 'function') {
        await permanentlyDeleteArchivedRecord(archiveDelete);
      } else {
        await deleteFromTable('deleted_records_archive', archiveDelete, b, 'Permanently delete this archived item?');
      }
      return;
    }

    // Admin report action variants
    const reportId = b.dataset.reportId || b.dataset.adminReportId;
    const reportAction = b.dataset.reportAction || b.dataset.staffReport || b.dataset.adminReport;
    if (reportId && reportAction) {
      const map = { approve: 'Approved', reject: 'Rejected', claimed: 'Claimed', returned: 'Returned' };
      const status = map[String(reportAction).toLowerCase()] || null;
      if (status) {
        e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
        await updateReportStatus(reportId, status, b);
        return;
      }
    }

    // Generic delete if data table/id exists
    if (b.dataset.action === 'delete' && b.dataset.table && b.dataset.id) {
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
      await deleteFromTable(b.dataset.table, b.dataset.id, b, 'Delete this item?');
      return;
    }
  }, true);
})();
