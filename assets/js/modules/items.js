  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const reportPayload = {
        ...
        image_url: imageUrl,
        status: 'Pending',
        reporter_name: currentProfile?.full_name || authUser.user_metadata?.full_name || '',
        reporter_email: currentProfile?.email || authUser.email || ''
      };

      let { data, error } = await sb.from('item_reports').insert(reportPayload).select().single();

      // If reporter_name/reporter_email columns are not installed yet, retry safely without them.
      ...
      const reportSessionUser = (typeof getUser === 'function' ? getUser() : {}) || {}; const reporterName = reportSessionUser?.name || reportSessionUser?.email || authUser?.email || 'A GCFind user';
      await createWorkflowNotification({
        recipientRole: 'faculty_staff',
        title: 'New Item Report Submitted',
        message: `${reporterName} submitted a new ${reportPayload.type || 'item'} report: ${itemName}. Please review it in the Staff Panel.`,
        type: 'report',
...
async function createWorkflowNotification({ recipientUserId = null, recipientEmail = null, recipientRole = null, title = 'Notification', message = '', type = 'info', relatedId = null } = {}) {
  ...
  try {
    if (typeof createNotification === 'function') {
      return await createNotification({ recipientUserId, recipientEmail, recipientRole, title, message, type, relatedId });
    }
    ...
    console.warn('Unable to create workflow notification:', err.message);
    return null;
  }
}
...
async function loadUserClaimNotifications() {
  ...
  if (reportIds.length) {
    const { data: reports } = await sb
      .from('item_reports')
      .select('id,item_name,location,status')
      .in('id', reportIds);
  ...
  const cards = [...notificationCards, ...claimCards];
