...
function appConfirm(message, options = {}) {
  ...
  if (!document.getElementById('gcfind-confirm-layer-style')) {
    const style = document.createElement('style');
    style.id = 'gcfind-confirm-layer-style';
    style.textContent = `
      #confirmOverlay {
      ...
      #confirmOverlay .gcfind-confirm-backdrop,
      #confirmOverlay .gcfind-confirm-shell {
        z-index: 2147483647 !important;
      }
    `;
    document.head.appendChild(style);
  }
  const confirmText = typeof options === 'string' ? options : (options.confirmText || 'Confirm');
  const tone = typeof options === 'object' ? (options.tone || 'default') : 'default';
  const primaryClass = tone === 'danger'
    ? 'rounded-xl bg-red-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-red-700'
    : 'rounded-xl bg-emerald-700 px-5 py-2.5 text-sm font-bold text-white hover:bg-emerald-800';
  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div class="gcfind-confirm-backdrop fixed inset-0 bg-slate-900/45 backdrop-blur-sm" style="z-index:2147483647!important;"></div>
      <div class="gcfind-confirm-shell fixed inset-0 flex items-center justify-center px-4" style="z-index:2147483647!important;">
        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-emerald-300 overflow-hidden">
          <div class="px-6 py-6">
            <h2 class="text-lg font-extrabold text-slate-900">Please confirm</h2>
            <p class="mt-4 text-sm text-slate-600">${escapeHtml(message)}</p>
          </div>
          <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4">
            <button type="button" class="rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50" data-confirm-cancel>Cancel</button>
            <button type="button" class="${primaryClass}" data-confirm-ok>${escapeHtml(confirmText)}</button>
          </div>
        </div>
      </div>
    `;
    ...
    overlay.firstElementChild?.addEventListener('click', () => close(false));
